...
class KehadiranDriverResource extends Resource
{
            ...
            ->schema([
                Section::make('Informasi Kehadiran Driver')
                    ->schema([
                        ...
                        Forms\Components\Select::make('unit_id')->label('Unit')->relationship('unit', 'nopol')->required()->searchable(),
                ...
                Section::make('Absensi Masuk')
                    ->schema([
                        ...
                        Forms\Components\Select::make('end_user_id')->label('Start User')->options(fn(Get $get) => EndUser::query()->when($get('project_id'), fn($query, $projectId) => $query->where('project_id', $projectId))->orderBy('name')->pluck('name', 'id'))->required()->searchable()->disabled(fn(Get $get) => blank($get('project_id')))->live(),
                ...
                Section::make('Absensi Check')
                    ->schema([
                        Forms\Components\Repeater::make('checks')
                            ...
                            ->relationship('checks')
                            ->schema([
                                Forms\Components\TextInput::make('location')->label('Lokasi Check'),
                                Forms\Components\DateTimePicker::make('created_at')->label('Waktu Check'),
                                Forms\Components\Textarea::make('note')->label('Catatan')->columnSpanFull()->nullable(),
                            ...
                            ->mutateRelationshipDataBeforeSaveUsing(function (array $data): array {
                                if (!empty($data['photo'])) {
                                    ...
                                    $data['photo'] = $ensureStoragePrefix($photo);
                                }
                ...
                Section::make('Absensi Keluar')
                    ->schema([
                        Forms\Components\Select::make('end_user_out')->label('End User')->options(fn(Get $get) => EndUser::query()->when($get('project_id'), fn($query, $projectId) => $query->where('project_id', $projectId))->orderBy('name')->pluck('name', 'id'))->required()->searchable()->disabled(fn(Get $get) => blank($get('project_id'))),
                ...
                Section::make('Informasi Lainnya')
                    ->schema([Forms\Components\Textarea::make('note')->label('Catatan')->maxLength(65535)->columnSpanFull(), Forms\Components\Textarea::make('note_admin')->label('Catatan Admin')->maxLength(65535)->columnSpanFull(), Forms\Components\Checkbox::make('is_complete')->label('Selesai')->default(false)])
                    ->columns(2),
            ])
            ->columns(1);
    }
    ...
    public static function table(Table $table): Table
    {
        ...
        ->paginated([10, 25, 50, 100])
            ...
            ->defaultSort('id', 'desc')
            ->filters([
                Tables\Filters\SelectFilter::make('project_id')->label('Filter Project')->relationship('project', 'name'),
                Tables\Filters\SelectFilter::make('user_id')
                    ...
                    ->query(function (Builder $query, array $data, \Filament\Tables\Contracts\HasTable $livewire): Builder {
                        ...
                        return $query->when($userId, fn(Builder $query, $userId) => $query->where('user_id', $userId))->when($projectId, function (Builder $query, $projectId): Builder {
                ...
                Tables\Filters\SelectFilter::make('month')
                    ...
                    ->options(function () {
                        $months = [];
                        $attendances = DriverAttendence::selectRaw('DISTINCT YEAR(date) as year, MONTH(date) as month')->orderBy('year', 'desc')->orderBy('month', 'desc')->get();
                    ...
                    ->schema([
                        ...
                        TextEntry::make('location_in')
                            ...
                            ->formatStateUsing(function ($state) {
                                if (blank($state) || !str_contains($state, ',')) {
                                    return '-';
                                }
                            ...
                            ->columnSpanFull(),
                        ImageEntry::make('photo_in')->label('Foto Masuk')->disk('public')->getStateUsing(fn($record) => self::normalizeStoragePath($record->photo_in))->size(300)->columnSpanFull(),
                    ])
                    ->columns(2),
                Section::make('Absensi Check')
                    ->schema([ViewEntry::make('checks.attendance_id')->label('Absensi Check')->view('filament.check-attendences')])
                    ->columns(2),
                ...
                Section::make('Absensi Keluar')
                    ->schema([
                        ...
                        TextEntry::make('location_out')
                            ...
                            ->columnSpanFull(),
                        ImageEntry::make('photo_out')->label('Foto Keluar')->disk('public')->getStateUsing(fn($record) => self::normalizeStoragePath($record->photo_out))->size(300)->columnSpanFull(),
                    ])
                    ->columns(2)
